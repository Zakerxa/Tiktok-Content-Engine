<template>
  ...
  <div class="login-root">
    <!-- Ambient brand orbs (same palette as Home / Login) -->
    ...
    <div class="login-card">
      ...
      <div class="brand-block">
        ...
        <h1 class="brand-name">Create your <span class="gradient-text">Account</span></h1>
        <p class="brand-tag">Start recapping in minutes — free tester plan included</p>
      </div>
      ...
      <form @submit.prevent="submit" class="login-form">
        <div class="field">
          ...
          <input
            ...
            required
            class="field-input"
            :class="{ 'field-input-error': form.errors.username }"
          />
        ...
        <button type="submit" :disabled="form.processing" class="btn-submit">
          ...
          <span>{{ form.processing ? 'Creating...' : 'Account ဖန်တီးမည်' }}</span>
        </button>
      </form>

      <p class="signin-row">
        Account ရှိပြီးသားလား?
        <a href="/login" class="link-accent">Login ဝင်ပါ</a>
      </p>

      <p class="footnote">
        Sign up ပြီးရင် Email confirmation link ပို့ပေးမည်၊ Verify လုပ်မှ အကောင့်ကို အပြည့်အဝ အသုံးပြုနိုင်ပါမည်။<br/>
      </p>
    </div>
...
.footnote { text-align: center; font-size: 11.5px; color: #475569; line-height: 1.7; margin-top: 22px; }
...
@media (prefers-reduced-motion: reduce) {
  .btn-google, .btn-submit, .home-link, .link-accent { transition: none !important; }
